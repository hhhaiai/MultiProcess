...
public class IUUBrodcastReciever extends BroadcastReceiver {
    ...
    boolean boo = true;
    List<IUUInfo> lexList = new ArrayList<>();
        ...
        EGQueue.execute(new SafeRunnable() {
            ...
            public void safeRun() {
                if (android.os.Build.BRAND.equals("LeEco")
                        && android.os.Build.MODEL.equals("LEX720")
                        && android.os.Build.VERSION.RELEASE.equals("6.0.1")
    ...
    private void filteringData(Context context) {
        ...
        if (lexList.size() == 1) {
            ...
            if (lexList.get(0).getActionType().equals("0")
                    && lexList.get(1).getActionType().equals("1")
                    && lexList.get(2).getActionType().equals("2")) {
                saveIuuinfo(context, lexList.get(2));
            } else if (lexList.get(0).getActionType().equals("1")
                    && lexList.get(1).getActionType().equals("0")
                    && lexList.get(2).getActionType().equals("2")) {
                List<InstalledAppInfo> list4 = InstalledAPPInfoManager.getAllApps(context);
                boolean bool = true;
                for (int i = 0; i < list4.size(); i++) {
                    if (list4.get(i).getApplicationPackageName()
                            .equals(lexList.get(2).getApplicationPackageName())) {
                        saveIuuinfo(context, lexList.get(2));
                        bool = false;
                        break;
                    }
            ...
            if (intent.getAction().equals(Intent.ACTION_PACKAGE_ADDED)) {
                ...
                try {
                    info.setApplicationName(
                            getProgramNameByPackageName(context, packageName));
                    if (pm.getPackageInfo(packageName, 0).versionName != null) {
                        info.setApplicationVersionCode(pm.getPackageInfo(
                ...
                List<InstalledAppInfo> list = manager.jsonToList(spUtil
                        .getAllAppForUninstall());
                for (int i = 0; i < list.size(); i++) {
                    if (packageName.equalsIgnoreCase(list.get(i).getApplicationPackageName())) {
                        ...
                        info.setApplicationVersionCode(
                                list.get(i).getApplicationVersionCode());
                        info.setActionType(flag);
                ...
                info.setApplicationPackageName(packageName);
                try {
                    info.setApplicationName(getProgramNameByPackageName(context,
                            packageName));
                    if (pm.getPackageInfo(packageName, 0).versionName != null) {
