...
public class EncryptUtils {
    ...
    private static String SP_EK_ID = "ek_id";
    private static String SP_CONTENT = "eguan";
    ...
    private static void clearEncryptKey(Context context) {
        ...
        SharedPreferences pref = context.getSharedPreferences(Constants.SPUTIL, Context.MODE_PRIVATE);
...
//        editor.putString(SP_EK_ID, "");
     ...
     * @return 加密后的字符串，可能为""
     */
    public static String encrypt(Context context, String src) throws EGDBEncryptException {
        try {
            if (TextUtils.isEmpty(mEncryptKey)) {
                init(context);
            }
            ...
            throw new EGDBEncryptException("encrypt");
        }
    }
     ...
     * @return 解密后的字符串
     */
    public static String decrypt(Context context, String str) throws EGDBEncryptException {
        try {
            ...
            throw new EGDBEncryptException("decrypt");
        }
    }
    ...
    public static void init(Context context) {
        ...
        if (TextUtils.isEmpty(id) || !id.contains("|")) {
            ...
            id = preID + "|" + serialNO;
            pref.edit().putString(SP_EK_ID, id).commit();
        }
...
//                //如果数据格式不对,则使用随机方案.不使用随机方案。使用后可能出现每次key都不一样的情况
//                String uuid = UUID.randomUUID().toString();
//                if (uuid.contains("-")) {
//                    String[] s = uuid.split("-");
//                    if (s.length > 0) {
...
//                pref.edit().putString(SP_EK_ID, id).commit();
            ...
            try {
                ...
                //如果数据格式不对,则使用随机方案
//                fID = String.valueOf(System.currentTimeMillis() / 10000);
                try {
