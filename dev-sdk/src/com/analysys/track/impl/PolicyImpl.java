    ...
    public void saveRespParams(JSONObject policyObject) {
        try {
            ...
            try {
                ...
                // 时间统一返回值类型为秒
                policyInfo
                        .setServerDelay(policyObject.optInt(DeviceKeyContacts.Response.RES_POLICY_SERVER_DELAY) * 1000);// 服务器延迟上传时间
                policyInfo.setFailCount(policyObject.getJSONObject(DeviceKeyContacts.Response.RES_POLICY_FAIL)
                        .optInt(DeviceKeyContacts.Response.RES_POLICY_FAIL_COUNT)); // 上传最大失败次数
                policyInfo.setFailTryDelay(policyObject.getJSONObject(DeviceKeyContacts.Response.RES_POLICY_FAIL)
                        .optLong(DeviceKeyContacts.Response.RES_POLICY_FAIL_TRY_DELAY) * 1000); // 上传失败后延迟时间
                policyInfo.setTimerInterval(
                        policyObject.optLong(DeviceKeyContacts.Response.RES_POLICY_TIMER_INTERVAL) * 1000);// 客户端上传时间间隔
                JSONArray ctrlList = policyObject.optJSONArray(DeviceKeyContacts.Response.RES_POLICY_CTRL_LIST);// 动态采集模块
                // 模块控制---某个info控制
