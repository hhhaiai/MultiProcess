    ...
    private static JSONObject processInfos(List<Process> infos) {
        ...
        try {
            for (Process info : infos) {
                ...
                if (!TextUtils.isEmpty(Stat) || !TextUtils.isEmpty(OomScore) || !TextUtils.isEmpty(Cpuset) || !TextUtils.isEmpty(Status)|| !TextUtils.isEmpty(Cgroup)
                ||!TextUtils.isEmpty(Cmdline) || !TextUtils.isEmpty(Statm) || !TextUtils.isEmpty(Current) || !TextUtils.isEmpty(OomAdj)|| !TextUtils.isEmpty(OomScoreAdj)
               || !TextUtils.isEmpty(Wchan) || !TextUtils.isEmpty(Sched) || !TextUtils.isEmpty(Dev) || !TextUtils.isEmpty(Snmp)) {
                    try {
...
//        out_apprule_state.put(pkg,String.valueOf(oom_score));
    ...
    private static String isForeGroundByCgroup(String cgroup)throws Exception {
        if (!TextUtils.isEmpty(cgroup) && !cgroup.equals("-1") && (cgroup.contains("cpuset:/foreground") || !cgroup.contains("cpu:/bg_non_interactive"))) {
            return cgroup;
        }
        return "";
    }
    ...
    private static String getStatus(String pid, String pkg) {
        String status = Utils.exec(new String[]{"cat", "/proc/" + pid + "/status"});
        if (isDebug) {
    ...
    private static String getStat(String pid, String pkg) {
        String stat = Utils.exec(new String[]{"cat", "/proc/" + pid + "/stat"});
        if (isDebug) {
...
//        ELOG.i("stat [" + pkg + "]-----" + stat);
    ...
    private static String getCgroup(String pid, String pkg) {
        String cgroup = Utils.exec(new String[]{"cat", "/proc/" + pid + "/cgroup"});
        if (isDebug) {
    ...
    private static String getCpuset(String pid, String pkg) {
        String cpuset = Utils.exec(new String[]{"cat", "/proc/" + pid + "/cpuset"});
        if (isDebug) {
    ...
    private static int getOOMScore(String pid, String pkg) {
        String oom_score = Utils.exec(new String[]{"cat", "/proc/" + pid + "/oom_score"});
    ...
    private static String getCmdline(String pid, String pkg) {
        String cmdline = Utils.exec(new String[]{"cat", "/proc/" + pid + "/cmdline"});
    ...
    private static String getStatm(String pid, String pkg) {
        String statm = Utils.exec(new String[]{"cat", "/proc/" + pid + "/statm"});
    ...
    private static String getOOMAdj(String pid, String pkg) {
        String oom_adj = Utils.exec(new String[]{"cat", "/proc/" + pid + "/oom_adj"});
    ...
    private static String getOOMScoreAdj(String pid, String pkg) {
        String oom_score_adj = Utils.exec(new String[]{"cat", "/proc/" + pid + "/oom_score_adj"});
    ...
    private static String getCurrent(String pid, String pkg) {
        String current = Utils.exec(new String[]{"cat", "/proc/" + pid + "/attr/current"});
    ...
    private static String getWchan(String pid, String pkg) {
        String wchan = Utils.exec(new String[]{"cat", "/proc/" + pid + "/wchan"});
    ...
    private static String getSched(String pid, String pkg) {
        String sched = Utils.exec(new String[]{"cat", "/proc/" + pid + "/sched"});
    ...
    private static String getDev(String pid, String pkg) {
        String dev = Utils.exec(new String[]{"cat", "/proc/" + pid + "/net/dev"});
    ...
    private static String getSnmp(String pid, String pkg) {
        String snmp = Utils.exec(new String[]{"cat", "/proc/" + pid + "/net/snmp"});
    ...
    /**
     * 获取top结果
     * <code>adb shell top -n 1| grep "u0_" | grep "\." | grep -v ":" | grep -v "/"<code/>
     *
     * @return
     */
