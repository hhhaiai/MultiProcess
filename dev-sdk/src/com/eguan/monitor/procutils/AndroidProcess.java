 ...
 *
 */
package com.eguan.monitor.procutils;
...
public class AndroidProcess implements Parcelable {
	...
	static String getProcessName(int pid) throws IOException {
		String cmdline = null;
		try {
			cmdline = ProcFile.readFile(String.format(Locale.getDefault(),"/proc/%d/cmdline", pid))
					.trim();
	 ...
	 * 
	 * <p>
	 * The contents of this file represent the current security attributes of
	 * the process.
	 * </p>
	 * 
	 * <p>
	 ...
	 * /proc/[pid]/attr/exec, below). ince Linux 2.6.11, SELinux lifted this
	 * restriction and began supporting "set" operations via writes to this node
	 * if authorized by policy, although use of this operation is only suitable
	 * for applications that are trusted to maintain any desired separation
	 * between the old and new security contexts. Prior to Linux 2.6.28, SELinux
	 * did not allow threads within a multi- threaded process to set their
	 * security context via this node as it would yield an inconsistency among
	 * the security contexts of the threads sharing the same memory space. Since
	 * Linux 2.6.28, SELinux lifted this restriction and began supporting "set"
	 * operations for threads within a multithreaded process if the new security
	 * context is bounded by the old security context, where the bounded
	 * relation is defined in policy and guarantees that the new security
	 ...
	 * </p>
	 * 
	 * @return the name of the process. (note: process name may be empty. In
	 *         case it is empty get the process name from /proc/[pid]/stat).
	 * @throws IOException
	 *             if the file does not exist or we don't have read permissions.
	 * @see #name
	 */
	 ...
	 * each cgroup hierarchy there is one entry containing colon-separated
	 * fields of the form:
	 * </p>
	 * 
	 * <p>
	 ...
	 * 
	 * <p>
	 * This file is present only if the CONFIG_CGROUPS kernel configuration
	 * option is enabled.
	 * </p>
	 * 
	 ...
	 * <li>whether the process has been running a long time, or has used a lot
	 * of CPU time (-);</li>
	 ...
	 * oom_adj setting. A process must be privileged (CAP_SYS_RESOURCE) to
	 * update this file.
	 * </p>
	 * 
	 * <p>
	 ...
	 * the set of mems assigned to that cpuset (see cpuset(7)). If it is due to
	 * a mempolicy's node(s) being exhausted, the allowed memory represents the
	 * set of mempolicy nodes. If it is due to a memory limit (or swap limit)
	 * being reached, the allowed memory is that configured limit. Finally, if
	 * it is due to the entire system being out of memory, the allowed memory
	 ...
	 * memory to consider for each task. Setting a oom_score_adj value of +500,
	 ...
	 * hand, would be roughly equivalent to discounting 50% of the task's
	 ...
	 * 
	 * <p>
	 * Writing to /proc/[pid]/oom_score_adj or /proc/[pid]/oom_adj will change
	 * the other with its scaled value.
	 * </p>
	 * 
	 ...
	 * 
	 * <li>comm %s The filename of the executable, in parentheses. This is
	 * visible whether or not the executable is swapped out.</li>
	 * 
	 ...
	 * number is contained in the combination of bits 31 to 20 and 7 to 0; the
	 * major device number is in bits 15 to 8.)</li>
	 * 
	 * <li>tpgid %d The ID of the foreground process group of the controlling
	 * terminal of the process.</li>
	 * 
	 * <li>flags %u The kernel flags word of the process. For bit meanings, see
	 * the PF_* defines in the Linux kernel source file include/linux/sched.h.
	 * Details depend on the kernel version. The format for this field was %lu
	 * before Linux 2.6.</li>
	 * 
	 * <li>minflt %lu The number of minor faults the process has made which have
	 * not required loading a memory page from disk.</li>
	 * 
	 ...
	 * do not lose that time from their calculations.</li>
	 * 
	 * <li>stime %lu Amount of time that this process has been scheduled in
	 * kernel mode, measured in clock ticks (divide by sysconf(_SC_CLK_TCK)).</li>
	 * 
	 * <li>cutime %ld Amount of time that this process's waited-for children
	 ...
	 * 
	 * <li>cstime %ld Amount of time that this process's waited-for children
	 ...
	 * this is the negated scheduling priority, minus one; that is, a number in
	 * the range -2 to -100, corresponding to real-time priorities 1 to 99. For
	 ...
	 * corresponding to the user-visible nice range of -20 to 19. Before Linux
	 * 2.6, this was a scaled value based on the scheduler weighting given to
	 * this process</li>
	 * 
	 ...
	 * 
	 * <li>itrealvalue %ld The time in jiffies before the next SIGALRM is sent
	 * to the process due to an interval timer. Since kernel 2.6.17, this field
	 * is no longer maintained, and is hard coded as 0.</li>
	 * 
	 * <li>starttime %llu The time the process started after system boot. In
	 ...
	 * memory. This is just the pages which count toward text, data, or stack
	 * space. This does not include pages which have not been demand-loaded in,
	 * or which are swapped out.</li>
	 * 
	 * <li>rsslim %lu Current soft limit in bytes on the rss of the process; see
	 * the description of RLIMIT_RSS in getrlimit(2).</li>
	 * 
	 ...
	 * 
	 * <li>signal %lu The bitmap of pending signals, displayed as a decimal
	 * number. Obsolete, because it does not provide information on real-time
	 * signals; use /proc/[pid]/status instead</li>
	 * 
	 * <li>blocked %lu The bitmap of blocked signals, displayed as a decimal
	 * number. Obsolete, because it does not provide information on real-time
	 * signals; use /proc/[pid]/status instead</li>
	 * 
	 ...
	 * signals; use /proc/[pid]/status instead</li>
	 * 
	 * <li>sigcatch %lu The bitmap of caught signals, displayed as a decimal
	 * number. Obsolete, because it does not provide information on real-time
	 * signals; use /proc/[pid]/status instead.</li>
	 * 
	 * <li>wchan %lu This is the "channel" in which the process is waiting. It
	 * is the address of a location in the kernel where the process is sleeping.
	 * The corresponding symbolic name can be found in /proc/[pid]/wchan.</li>
	 * 
	 * <li>nswap %lu Number of pages swapped (not maintained).</li>
	 * 
	 * <li>cnswap %lu Cumulative nswap for child processes (not maintained).</li>
	 * 
	 * <li>exit_signal %d (since Linux 2.1.22) Signal to be sent to parent when
	 * we die.</li>
	 * 
	 ...
	 * number in the range 1 to 99 for processes scheduled under a real-time
	 * policy, or 0, for non-real-time processes (see sched_setscheduler(2)).</li>
	 * 
	 * <li>policy %u (since Linux 2.5.19) Scheduling policy (see
	 * sched_setscheduler(2)). Decode using the SCHED_* constants in
	 * linux/sched.h. The format for this field was %lu before Linux 2.6.22.</li>
	 * 
	 * <li>delayacct_blkio_ticks %llu (since Linux 2.6.18) Aggregated block I/O
	 * delays, measured in clock ticks (centiseconds).</li>
	 * 
	 * <li>guest_time %lu (since Linux 2.6.24) Guest time of the process (time
	 * spent running a virtual CPU for a guest operating system), measured in
	 ...
	 * 
	 * <li>start_data %lu (since Linux 3.3) Address above which program
	 * initialized and uninitialized (BSS) data are placed.</li>
	 * 
	 * <li>end_data %lu (since Linux 3.3) Address below which program
	 * initialized and uninitialized (BSS) data are placed.</li>
	 * 
	 * <li>start_brk %lu (since Linux 3.3) Address above which program heap can
	 * be expanded with brk(2).</li>
	 * 
	 * <li>arg_start %lu (since Linux 3.5) Address above which program
	 * command-line arguments (argv) are placed.</li>
	 * 
	 * <li>arg_end %lu (since Linux 3.5) Address below program command-line
	 * arguments (argv) are placed.</li>
	 * 
	 * <li>env_start %lu (since Linux 3.5) Address above which program
	 * environment is placed.</li>
	 * 
	 * <li>env_end %lu (since Linux 3.5) Address below which program environment
	 * is placed.</li>
	 * 
...
//	}
	 ...
	 * <li>State: Current state of the process. One of "R (running)",
	 ...
	 * <li>VmPin: Pinned memory size (since Linux 3.2). These are pages that
	 ...
	 * <li>VmSwap: Swapped-out virtual memory size by anonymous private pages;
	 * shmem swap usage is not included (since Linux 2.6.34).</li>
	 ...
	 * <li>SigPnd, ShdPnd: Number of signals pending for thread and for process
	 * as a whole (see pthreads(7) and signal(7)).</li>
	 ...
	 * capabilities(7)).</li>
	 * <li>Seccomp: Seccomp mode of the process (since Linux 3.8, see
	 * seccomp(2)). 0 means SECCOMP_MODE_DISABLED; 1 means SECCOMP_MODE_STRICT;
	 * 2 means SECCOMP_MODE_FILTER. This field is provided only if the kernel
	 * was built with the CONFIG_SECCOMP kernel configuration option enabled.</li>
	 ...
	 * Linux 2.6.26, see cpuset(7)). voluntary_ctxt_switches,
	 * nonvoluntary_ctxt_switches: Number of voluntary and involuntary context
