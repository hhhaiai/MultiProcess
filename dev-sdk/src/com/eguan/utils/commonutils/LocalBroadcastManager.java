 ...
 * your process. This has a number of advantages over sending global broadcasts
 * with {@link android.content.Context#sendBroadcast}:
 * <ul>
...
public final class LocalBroadcastManager {
    ...
    public void registerReceiver(BroadcastReceiver receiver, IntentFilter filter) {
        synchronized (mReceivers) {
            ...
            if (filters == null) {
                filters = new ArrayList<ReceiverRecord>(1);
     ...
     *
     * @param intent
     *            The Intent to broadcast; all receivers matching this Intent will
     *            receive the broadcast.
     * @return Returns true if the intent has been scheduled for delivery to one or
     *         more broadcast receivers. (Note tha delivery may not ultimately take
     *         place if one of those receivers is unregistered before it is
     *         dispatched.)
     * @see #registerReceiver
     */
    public boolean sendBroadcast(Intent intent) {
        synchronized (mReceivers) {
            ...
            if (Constants.FLAG_DEBUG_INNER) {
                EgLog.v("Resolving type " + type + " scheme " + scheme + " of intent " + intent);
            }
            ArrayList<ReceiverRecord> entries = mActions.get(intent.getAction());
            if (entries != null) {
                // if (debug) Log.v(TAG, "Action list: " + entries);
                if (Constants.FLAG_DEBUG_INNER) {
                    EgLog.v("Action list: " + entries);
                }
                ...
                for (int i = 0; i < entries.size(); i++) {
                    ...
                    if (Constants.FLAG_DEBUG_INNER) {
                        EgLog.v("Matching against filter " + receiver.filter);
                    }
                    if (receiver.broadcasting) {
                        ...
                        // }
                        if (Constants.FLAG_DEBUG_INNER) {
                            EgLog.v("  Filter's target already added");
                        }
                        continue;
                    }
                    ...
                    if (match >= 0) {
                        ...
                        if (Constants.FLAG_DEBUG_INNER) {
                            EgLog.v("  Filter matched!  match=0x" + Integer.toHexString(match));
                        }
                        if (receivers == null) {
                        ...
                        if (debug) {
                            ...
                            if (Constants.FLAG_DEBUG_INNER) {
                                EgLog.v("  Filter did not match: " + reason);
                            }
                        }
                    }
                }
    ...
    /**
     * Like {@link #sendBroadcast(Intent)}, but if there are any receivers for the
